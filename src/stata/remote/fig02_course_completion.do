*==============================================================================*
* fig02_course_completion.do                (was 00023E2_analyses_remote.do)
*
* GENERATED BY src/analyses/remote/tools/make_23_jobs.py
*           -- DO NOT EDIT BY HAND, REGENERATE
*
* Purpose : Paper FIGURE 2, PANEL B. Average marginal effect of county
*           course supply on Pr(completion of an integration course),
*           evaluated at eight durations of stay: 0, 6, 12, 18, 24, 30, 36
*           and 42 months (stay_dur is in months/12, so 0 to 3.5 in steps of
*           0.5). This is Model 3.2 of paper Table 4 read as a slope over
...
* Source  : 00023_mult_analyses.do:957-989 (the mimrgns + marginsplot block)
* Runs    : SOEPremote (soep-exec@diw.de)
* Depends : impute_sample.do -> $mydata/msuemer/sample_imputed
* Saves   : $mydata/msuemer/fig02_crs  (mi estimation results, write-only)
* Result  : results/comparisons/fig02_marginal_effects.md
*           results/figures/fig02_panel_b.{png,svg}, drawn locally by
...
*   m_0032p  the authors' own estimates, written by their Table 4 block
*   m_0032e  the in-memory esample marker this job creates (port's name)
*   fig02_crs  the .ster file this job writes on the server (migration's name)
...
* The authors call `mimrgns using $output/m_0032p, esample(m_0032p)`,
...
* equal tab04_intermediate_outcomes.do's Model 3.2 column exactly. That
...
* the cold rebuild. Writes fig02_crs -- a name that has never existed on the
* server. The pre-migration job saved to m_0032e instead, and that copy may
...
* (5) `saving($output/m_0032p)` -> `saving($mydata/msuemer/fig02_crs)`; see
...
*     effect(stay_dur) = 100 * (0.0434 + stay_dur * -0.0141)
* i.e. at 0, 6 ... 42 months: 4.34 3.64 2.93 2.23 1.52 0.82 0.11 -0.59
...
*   y axis   ysc(r(-4.5 6.5)) ylabel(-4 (2) 6)
...
*   title    "Panel B. Pr(completion of an integration course) / in percentage points, Model 3.2"
...
*
* ---------------- WHAT TO CHECK ----------------
*   the coefficient table -> must match the Model 3.2 column of
...
*   `display "23E2 ok"` at the end -- with no dataset saved by this job,
...
*       src/stata/remote/fig02_course_completion.do
*==============================================================================*
...
capture soepdrop m_0032e
mi estimate, post dots ///
    saving($mydata/msuemer/fig02_crs, replace) esample(m_0032e): ///
    reg int_course_finished $controls_fv ///
    c.stay_dur##c.z_ratio_courseoport ///
    [pweight=weight_p], vce(cluster pid)

mimrgns using $mydata/msuemer/fig02_crs, esample(m_0032e) ///
